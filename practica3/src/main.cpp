...
int main(int argc, char const *argv[]) {
  ...
  try {
    ...
    write_errLog("In MAIN PROCEDURE. Exiting ... ");
    write_errLog(e.what());
    _Exit(1);
  }
}
...
void client_life_cycle(Client& c) {
  try {
    ...
    write_errLog("In CLIENT " + std::to_string(c.get_nClient()) + " Exiting ...");
    write_errLog(e.what());
    _Exit(1);
  }
}
...
void ticket_office_life_cycle(Ticket_Office& ticket_office) {
  try {
    ...
    write_errLog("In TICKET OFFICE. Exiting ... ");
    write_errLog(e.what());
    _Exit(1);
  }
}
...
void payment_system_life_cycle(Payment_system& payment_system) {
  try {
    ...
    write_errLog("In PAYMENT SYSTEM. Exiting ... ");
    write_errLog(e.what());
    _Exit(1);
  }
}
...
void food_stand_life_cycle(Food_Stand& food_stand) {
  try {
    ...
    write_errLog("In FOOD STAND. Exiting ... ");
    write_errLog(e.what());
    _Exit(1);
  }
}
...
void supplier_life_cycle(Supplier &supplier) {
  try {
    ...
    write_errLog("In SUPPLIER. Exiting ... ");
    write_errLog(e.what());
    _Exit(1);
  }
}
